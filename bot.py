        ...
        if query in set(session_list_dic.keys()):
            ...
            if query == "crisis":
                ...
                session_info = f"\n\nЭто <b>закрытая группа</b> с ограниченым количеством участников (осталось <b>{session_item['limit']}</b> мест). Записаться можно отправив любую предложенную сумму в качестве пожертвования.\n\n" \
                              ...
                              f"* <b>ВАЖНО</b> - после успешного перевода вернитесь в чат бота и нажмите кнопку <b>Подтвердить</b>. Так мы сможем отследить Ваш донат и прислать ссылку для подключения. Дождитесь подтверждения, обычно это занимает несколько минут\n" \
